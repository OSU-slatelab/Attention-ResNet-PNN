from resnet_attention_module import *
...
class Train(object):
    ...
    def build_train_validation_graph(self):
        ...
        logits = inference_attention_module(
            self.input_placeholder, is_training=self.is_training_placeholder)
        ...
        #self.predictions = predictions  # TODO: remove self.predictions
    ...
    def train(self):
        ...
        if FLAGS.is_use_ckpt is True:
            log.debug ('Restoring from checkpoint... {}'.format(FLAGS.attention_ckpt_path))
            saver.restore(sess, FLAGS.attention_ckpt_path)
            log.debug ('Restored model ...')
        else:
            sess.run(init)
        ...
        log.debug ("Starting Validation Loss: {} | Staring Validation Error: {}".format(validation_loss_value, validation_error_value))
        log.debug ('------------------------------------------------------------')
        log.debug('\n')

        lr = FLAGS.init_lr_att
        half_lr = 0   # -----------------------------------------
        #stop_training = False
        pre_batch_feat = np.zeros((128, 40, 11, 3))
        pre_batch_labl = np.zeros((128))
        ...
        for epoch in range(FLAGS.train_epoch):   # -----------------------------
            ...
            iter = 0
            #for x in range(100):
            log.debug ('Start training Epoch {}'.format(epoch))
            log.debug ('The Learning rate of the epoch is: {}'.format(lr))
            while not end_epoch:
                #log.debug (pre_batch_feat)
                #log.debug (pre_batch_labl)
                train_batch_data, train_batch_labels, end_epoch = dispenser.get_batch()

                #if np.array_equal(pre_batch_feat, train_batch_data) and np.array_equal(pre_batch_labl,
                #        train_batch_labels):
                #    log.debug ("training_batch_data: {}".format(train_batch_data))
                #    log.debug ("training_batch_label: {}".format(train_batch_labels))
                #    #log.debug ("prediction_batch: {}".format(predictions))
                #    log.debug (50*"=")
                pre_batch_feat = train_batch_data
                pre_batch_labl = train_batch_labels
                ...
                _, _, train_loss_value, train_error_value = sess.run([self.train_op, self.train_ema_op, #self.predictions, #remove prediction
                ...
                log.debug("Epoch {}, Iter {}: training loss: {}, training error: {}, end_epoch: {}".format(epoch, iter,
            ...
            dispenser.reset()
            log.debug ("Time spend for epoch {}: {}".format(epoch, str(datetime.timedelta(seconds=duration))))
            log.debug ('\n')

            # Check the validation
            # loss first
            if not math.isnan(train_loss_value):
                log.debug ('Strat Validation ...')
            ...
            log.debug ("Validation Loss: {} | Validation Error: {}".format(validation_loss_value_new,
            ...
            log.debug ('Validation top1 error = %.4f' % validation_error_value_new)
            log.debug ('Validation loss = {}'.format(validation_loss_value_new))
            log.debug ('----------------------------')
            ...
            if validation_loss_value_new < validation_loss_value:
                ...
                best_model = epoch
                checkpoint_path = os.path.join(FLAGS.attention_train_dir, 'model.ckpt')
            else:
                checkpoint_path = os.path.join(FLAGS.attention_train_dir, 'model.ckpt_reject')
            ...
            if validation_loss_value_pre != validation_loss_value and \
               (validation_loss_value_pre - validation_loss_value) / validation_loss_value_pre \
                ...
                lr != FLAGS.init_lr_att:
                log.debug ("(Vali_loss_pre - Vali_loss) / Vali_loss_pre: ({} - {}) / {} = {}".format(validation_loss_value_pre,
                    validation_loss_value, validation_loss_value_pre, (validation_loss_value_pre -
                        validation_loss_value) / validation_loss_value_pre))
                log.debug ('Stop training: Validation Loss decrease is too small')
                break
            if (validation_loss_value_pre - validation_loss_value) / validation_loss_value_pre \
                    < FLAGS.halving_lr_bar:
                half_lr = 1
            if half_lr == 1:
                if lr > FLAGS.init_lr_att * 0.001:
                    lr = FLAGS.lr_decay_factor_attention * lr
                else:
                    lr = FLAGS.lr_decay_factor * lr
                log.debug ("(Vali_loss_pre - Vali_loss) / Vali_loss_pre: ({} - {}) / {} = {}".format(validation_loss_value_pre,
                    validation_loss_value, validation_loss_value_pre, (validation_loss_value_pre -
                        validation_loss_value) / validation_loss_value_pre))
                log.debug ('Learning rate decayed to {}'.format(lr)) 
            log.debug ('------------------------------------------')
            log.debug ('\n')

            if best_model != epoch:
                ckpt_path = os.path.join(FLAGS.attention_train_dir, 'model.ckpt-' + str(best_model))
                log.debug ('Restoring from checkpoint: {}'.format(ckpt_path))
                saver.restore(sess, ckpt_path)
                log.debug ('Restored from checkpoint ...')

        #df = pd.DataFrame(data={'epoch': step_list, 'validation_error': val_error_list})
        #df.to_csv(FLAGS.attention_train_dir + '/' + str(epoch) + '_validation_error.csv')
    ...
    def test(self):
        ...
        #test_lablreader = kaldiIO.LabelReader(aligdir + '/clean_lab.scp')
        ...
        logits = inference_attention_module(self.test_input_placeholder, is_training=self.is_training_placeholder)
        ...
        log.debug ('Model restored from: {} '.format(FLAGS.test_ckpt_path))
        ...
        #log.debug ("zero_logit: {}".format(zero_logit_array))
        #log.debug ("zero_logit_shape: {}".format(zero_logit_array.shape))
        #zero_pred = zero_logit_array[0]
        #log.debug (zero_pred.shape)
        #log.debug ("!!!!!!!!!!!!!!!!!!!!!!!!!!")
        count = 0
        for i in range(dispenser.num_utt()):
            ...
            for step in range(num_batches):
                ...
                #log.debug ("batch {}: {}".format(step, test_batch))
                #log.debug (50 * "=")
            ...
            log.debug ("{}th utterance: {} -----------".format(i, utt_id))
            log.debug ("num_frame: {}".format(num_frames))
            log.debug ("utt_logits length: {}".format(utt_logits_array.shape[0]))
            log.debug ("num_zeros_frames: {}".format(utt_logits_array[num_frames:].shape))
            #log.debug (utt_logits_array)
            #log.debug (50 * "-")
            #log.debug (utt_logits_array[num_frames:])
            ...
            log.debug ("num_nonzeros_frames: {}".format(utt_logits_array.shape))
            uids.append(utt_id)
            total_logits_array.append(utt_logits_array)

        log.debug ("num_utt: {}, {}".format(len(uids), len(total_logits_array)))
    ...
    def full_validation(self, loss, top1_error, vali_dispenser, session):
        ...
        while not end_vali_set:
            ...
            #log.debug (vali_feat_batch.shape)
            #log.debug ([int(FLAGS.validation_batch_size), INPUT_HEIGHT, INPUT_WIDTH, INPUT_DEPTH])
            ...
            #log.debug ('Iter {} - Validation Batch Loss: {} | Validation Batch Error: {}'.format(count, loss_value, top1_error_value))
